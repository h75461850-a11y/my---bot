...
# 명단 출력용 리스트 (update_admin_list 명령에서 사용)
ROLES_LIST = {
...
async def update_admin_list(interaction: discord.Interaction):
    ...
    for d in ["총무", "보안", "안내", "뉴관", "홍보", "기획", "내전"]:
        team_id = DEPT_DATA[d]["팀원"]
        team_m = guild.get_role(team_id).members if guild.get_role(team_id) else []
        ...
        lines.append(f"<@&{team_id}> : {staff if staff else f'<@&{VACANCY_ROLE_ID}>'}")
...
async def team_pass(interaction: discord.Interaction, 대상자: discord.Member):
    ...
    # 각 팀장 역할 ID에 따른 지급 역할 매핑
    mapping = {
        DEPT_DATA["총무"]["팀장"]: [DEPT_DATA["총무"]["팀원"], INTERN_ROLE_ID, 1475272064234553456], # 설야관리자 포함
...
async def give_roles(interaction: discord.Interaction, 유저: discord.Member, 이름: str, 나이: str, 성별: str, 경로: str):
    ...
    log_embed = discord.Embed(title="<a:emoji_01:1491559934523867146> 뉴페기록", color=discord.Color.blue())
...
bot.run("MTQ3OTEyMjQ3MjIxMjM2NTM4NA.GcrUFk.P9zeD1O40V8a2HBIPAnUmF0pw084Wca2fc3j9c")